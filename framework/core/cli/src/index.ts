...
export const packageDescription =
  "Terminal command surface for platform scaffolding, AI operations, registry workflows, and system understanding tools." as const;
...
export async function runCli(argv: string[], io: CliIo): Promise<number> {
  ...
  try {
    ...
    if (command === "make" && subcommand === "ai-pack") {
      ...
      if (!packId) {
        return commandFailure(io, "Missing required flag --id for `platform make ai-pack`.");
      }
...
export function scaffoldAiPack(cwd: string, packId: string): string {
  ...
  const docPack = createUnderstandingDocPack({
    ...
    description: "AI pack scaffold generated by platform CLI.",
...
function createCliPromptPolicy() {
  return defineGuardrailPolicy({
    id: "platform-cli.prompt-validation",
