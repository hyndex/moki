...
export async function seedAll(opts: { force?: boolean } = {}): Promise<void> {
  ...
  if (row.c > 0 && !opts.force) {
    ...
    const combined = { ...crmExt, ...salesExt, ...acctExt, ...invExt, ...hrExt, ...supExt, ...bookExt, ...fsExt };
  ...
  const bookExt = seedBookingExtended();
  const fsExt = seedFieldServiceExtended();
  const all = { ...crm, ...factory, ...extended, ...crmExt, ...salesExt, ...acctExt, ...invExt, ...hrExt, ...supExt, ...bookExt, ...fsExt };
