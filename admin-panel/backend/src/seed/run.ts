...
export async function seedAll(opts: { force?: boolean } = {}): Promise<void> {
  ...
  if (row.c > 0 && !opts.force) {
    ...
    const combined = { ...crmExt, ...salesExt, ...acctExt, ...invExt, ...hrExt, ...supExt, ...bookExt, ...fsExt, ...prjExt };
  ...
  const fsExt = seedFieldServiceExtended();
  const prjExt = seedProjectsExtended();
  const all = { ...crm, ...factory, ...extended, ...crmExt, ...salesExt, ...acctExt, ...invExt, ...hrExt, ...supExt, ...bookExt, ...fsExt, ...prjExt };
