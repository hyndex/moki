...
const OPP_STAGES = ["discovery", "qualification", "proposal", "negotiation", "won", "lost"];
const STAGE_PROB = {
    discovery: 10,
    qualification: 25,
    proposal: 50,
...
const CAMPAIGN_TYPES = ["email", "event", "webinar", "paid-ads", "content", "partner", "direct-mail"];
...
const personName = (i) => `${pick(FIRST_NAMES, i)} ${pick(LAST_NAMES, i + 2)}`;
const daysAgo = (n) => new Date(Date.now() - n * 86_400_000).toISOString();
const code = (prefix, i, pad = 4) => `${prefix}-${String(i + 1).padStart(pad, "0")}`;
function seedLeads(n) {
...
function seedOpportunities(n) {
    return Array.from({ length: n }, (_, i) => {
        ...
        return {
            id: `opp_${i + 1}`,
            code: code("OP", i),
            name: `${pick(LEAD_COMPANIES, i)} — ${pick(["expansion", "renewal", "new logo", "upsell", "cross-sell"], i)}`,
...
function seedAppointments(n) {
    return Array.from({ length: n }, (_, i) => {
        const type = pick(["discovery-call", "demo", "follow-up", "onboarding", "check-in"], i);
        const status = pick(["proposed", "confirmed", "completed", "cancelled", "no-show"], i);
        return {
            id: `appt_${i + 1}`,
            code: code("AP", i),
...
function seedContracts(n) {
    const CONTRACT_STATUSES = ["draft", "sent", "signed", "active", "renewed", "expired", "terminated"];
    return Array.from({ length: n }, (_, i) => {
        ...
        return {
            ...
            signedAt: status === "signed" || status === "active" || status === "renewed"
                ? daysAgo(300 - (i * 5) % 300)
                : undefined,
...
export const leadResource = tagSeed(defineResource({
    id: "crm.lead",
    singular: "Lead",
    plural: "Leads",
    schema: LeadSchema,
...
export const opportunityResource = tagSeed(defineResource({
    id: "crm.opportunity",
...
export const campaignResource = tagSeed(defineResource({
    id: "crm.campaign",
    singular: "Campaign",
    plural: "Campaigns",
...
export const appointmentResource = tagSeed(defineResource({
    id: "crm.appointment",
...
export const contractResource = tagSeed(defineResource({
    id: "crm.contract",
    singular: "Contract",
    plural: "Contracts",
...
export const competitorResource = tagSeed(defineResource({
    id: "crm.competitor",
    singular: "Competitor",
    plural: "Competitors",
...
export const marketSegmentResource = tagSeed(defineResource({
...
export const salesStageResource = tagSeed(defineResource({
    id: "crm.sales-stage",
...
const taskResource = tagSeed(defineResource({
    id: "crm.task",
    singular: "Task",
    plural: "Tasks",
    schema: TaskSchema,
...
const callResource = tagSeed(defineResource({
    id: "crm.call",
    singular: "Call",
    plural: "Calls",
    schema: CallSchema,
...
export const CRM_EXTENDED_RESOURCES = [
    leadResource,
    opportunityResource,
    campaignResource,
    appointmentResource,
...
export const CRM_EXTENDED_VIEWS = [
    leadListView,
    leadFormView,
    leadDetailView,
    opportunityListView,
