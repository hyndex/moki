...
const code = (p, i, pad = 4) => `${p}-${String(i + 1).padStart(pad, "0")}`;
function tag(r, rows) {
    r.__seed = rows;
    return r;
}
...
export const salesOrderResource = tag(defineResource({
    ...
    status: pick(["submitted", "partially-delivered", "delivered", "invoiced", "closed", "cancelled"], i),
...
export const SALES_EXTENDED_RESOURCES = [
    salesOrderResource,
...
export const SALES_EXTENDED_VIEWS = [
    defineListView({
