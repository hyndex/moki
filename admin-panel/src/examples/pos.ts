...
const salesByTerminalReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { terminal: string; sales: number; revenue: number; items: number }>();
...
const salesByHourReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const sales = await fetchAll(resources, "pos.sale");
    const byHour = new Map<number, { hour: number; sales: number; revenue: number }>();
    for (const s of sales) {
...
const topProductsReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { sku: string; name: string; qty: number; revenue: number }>();
