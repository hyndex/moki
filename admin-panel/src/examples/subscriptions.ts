...
const mrrByPlanReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { plan: string; customers: number; mrr: number; avgMrr: number }>();
    for (const s of subs) {
...
const churnReport: ReportDefinition = {
...
const renewalsReport: ReportDefinition = {
