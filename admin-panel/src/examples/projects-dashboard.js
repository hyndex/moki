    ...
    async execute({ resources }) {
        ...
        return {
            ...
            chart: { kind: "line", label: "Velocity trend",
                from: (rs) => rs.map((r) => ({ label: r.name, value: r.velocity })) },
        };
    },
};
    ...
    async execute({ resources }) {
        ...
        return {
            ...
            chart: { kind: "line", label: "Burndown",
                from: (rs) => rs.map((r) => ({ label: r.day, value: r.open })) },
        };
    },
};
export const PROJECTS_REPORTS = [
