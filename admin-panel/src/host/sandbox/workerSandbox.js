...
function attachWorkerRpc(worker, host, plugin) {
    const cleanups = [];
    const onMessage = async (ev) => {
        const m = ev.data;
        if (!m?.__gutuSandbox || typeof m.id !== "number" || !m.method)
            return;
        try {
...
async function dispatchWorkerRpc(host, plugin, method, params) {
    void host;
    void plugin;
    ...
    if (method.startsWith("log.")) {
        const level = method.slice("log.".length);
