...
export async function spawnIframeSandbox(args) {
...
function attachRpcHandler(iframe, host, plugin) {
    const disposers = [];
    ...
    const onMessage = async (ev) => {
        ...
        try {
            ...
            iframe.contentWindow?.postMessage({ __gutuSandbox: true, id: m.id, result }, "*");
        }
        catch (err) {
            iframe.contentWindow?.postMessage({
                __gutuSandbox: true,
...
async function dispatchRpc(host, _plugin, method, params, disposeMap) {
