...
export function resolveRoute(path, registry) {
    ...
    if (clean === "/" || clean === "") {
        const firstDashboard = Object.values(registry.views).find((v) => v.type === "dashboard");
        if (firstDashboard)
            return {
                path: "/",
                view: firstDashboard,
...
function collectNavPaths(nav, acc = []) {
    for (const item of nav) {
        if (item.path)
            acc.push({ path: item.path, view: item.view, id: item.id });
        if (item.children)
            collectNavPaths(item.children, acc);
    }
    return acc;
}
function findViewForNav(nav, registry) {
    // 1. Explicit: nav.view declared.
    if (nav.view && registry.views[nav.view])
        return registry.views[nav.view];
    // 2. By nav id.
    if (registry.views[nav.id])
        return registry.views[nav.id];
    ...
    const candidates = [nav.id, lastSeg].filter(Boolean);
    for (const candidate of candidates) {
        const hit = Object.values(registry.views).find((v) => (v.type === "list" ||
            v.type === "dashboard" ||
            v.type === "custom" ||
            v.type === "kanban") &&
    ...
    for (const candidate of candidates) {
        const hit = Object.values(registry.views).find((v) => v.id.includes(candidate));
        if (hit)
            return hit;
    }
    return null;
}
function classify(view, base, remainder) {
