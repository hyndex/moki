    ...
    switch (leaf.op) {
        ...
        case "between": {
            if (!Array.isArray(leaf.value) || leaf.value.length !== 2)
                return false;
            if (typeof v === "number")
                return v >= leaf.value[0] && v <= leaf.value[1];
            if (typeof v === "string")
                return v >= leaf.value[0] && v <= leaf.value[1];
            return false;
        }
...
export function evalFilter(tree, record) {
    if ("and" in tree)
        return tree.and.every((c) => evalFilter(c, record));
    if ("or" in tree)
        return tree.or.some((c) => evalFilter(c, record));
    return evalLeaf(tree, record);
}
export function resolveTimeRange(range) {
    const now = new Date();
    if (!range || range.kind === "all")
...
function aggregate(records, fn, field) {
    if (fn === "count")
...
export async function computeAggregation(spec, resources) {
