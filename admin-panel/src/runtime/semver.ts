...
export function satisfies(version: string, range: string): boolean {
  ...
  for (const orPart of orParts) {
    if (satisfiesAnd(v, orPart)) return true;
  }
  return false;
}

function satisfiesAnd(v: ParsedVersion, andExpr: string): boolean {
  // Expand ^ and ~ first
  const tokens = andExpr.trim().split(/\s+/).flatMap(expandShorthand);
  if (tokens.length === 0) return true;
  for (const token of tokens) {
    if (!satisfiesComparator(v, token)) return false;
  }
  return true;
}
...
function satisfiesComparator(v: ParsedVersion, token: string): boolean {
