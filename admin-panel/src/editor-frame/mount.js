...
export async function mountAdapter(kind, container, doc, initialBytes, recordId) {
    switch (kind) {
...
/* ---------------- Univer Sheet ---------------- */
async function mountUniverSheet(container, doc, initialBytes) {
    const [core, ui, design, render, formula, sheets, sheetsUi, docs, docsUi, sheetsLocale, uiLocale, designLocale, docsLocale,] = await Promise.all([
    ...
    const univer = new core.Univer({
        theme: resolveUniverTheme(design),
    ...
    if (!map.get("snapshot"))
        map.set("snapshot", JSON.stringify(snapshot));
    return {
        async exportSnapshot() {
            const txt = map.get("snapshot") ?? JSON.stringify(snapshot);
...
/* ---------------- Univer Doc ---------------- */
async function mountUniverDoc(container, doc, initialBytes) {
    const [core, ui, design, render, docs, docsUi, uiLocale, designLocale, docsLocale,] = await Promise.all([
        import("@univerjs/core"),
        import("@univerjs/ui"),
        import("@univerjs/design"),
        import("@univerjs/engine-render"),
...
/* ---------------- Univer Slides ---------------- */
async function mountUniverSlides(container, doc, initialBytes) {
    const [core, ui, design, render, docs, docsUi, slides, slidesUi, uiLocale, designLocale, docsLocale, slidesLocale,] = await Promise.all([
        import("@univerjs/core"),
        import("@univerjs/ui"),
        import("@univerjs/design"),
        import("@univerjs/engine-render"),
...
async function mountBlockEditor(container, doc, recordId) {
    ...
    let handle = null;
    let handleResolved = () => { };
    const handlePromise = new Promise((r) => { handleResolved = r; });
    const setHandle = (h) => {
        if (h) {
            handle = h;
            handleResolved(h);
    ...
    const holder = { status: "loading", error: null, listeners: new Set() };
    ...
    if (collab) {
        collab.subscribe((info) => {
            try {
                window.parent?.postMessage({ type: "editor-frame-presence", peers: info.peers, status: info.status }, window.location.origin);
            }
            catch { /* tolerate */ }
        });
    }
...
function mountWhiteboard(container, doc, recordId) {
    const yShapes = doc.getArray("whiteboard-shapes");
    ...
    for (const tool of ["select", "rect", "ellipse", "text", "pen", "arrow"]) {
            ...
            if (hit) {
                ...
                yShapes.forEach((s) => {
                    if (selectedIds.has(s.id))
                        offs.set(s.id, { dx: x - s.x, dy: y - s.y });
                });
    ...
    const mu = () => {
        ...
        if (drag.mode === "marquee" && marquee) {
            ...
            const meaningful = s.kind === "pen" ? (s.points?.length ?? 0) > 1 :
    ...
    const deleteSelection = () => {
        ...
        doc.transact(() => {
            ...
            for (let i = yShapes.length - 1; i >= 0; i--) {
                if (selectedIds.has(yShapes.get(i).id))
                    yShapes.delete(i, 1);
            }
        });
        selectedIds = new Set();
...
/* ---------------- Helpers ---------------- */
function parseInitialOrBlank(bytes, blank) {
...
function resolveUniverTheme(design) {
