import type { FilterLeaf, FilterOp, FilterTree } from "@/contracts/saved-views";
...
export function evalFilter(
  ...
  if ("and" in tree) {
    if (tree.and.length === 0) return true;
    return tree.and.every((c) => evalFilter(record, c, now));
  }
  if ("or" in tree) {
    if (tree.or.length === 0) return true;
...
function relativeRange(op: FilterOp, now: Date): [number, number] | null {
  ...
  switch (op) {
    ...
    case "qtd": {
      ...
      const end =
        op === "qtd" ? todayEnd : new Date(y, qStart + 3, 1).getTime();
      return [start, end];
    }
    case "last_quarter": {
...
export function describeFilter(tree: FilterTree | undefined): string {
  if (!tree) return "No filter";
  if ("and" in tree) return tree.and.map(describeFilter).join(" AND ");
  if ("or" in tree) return tree.or.map(describeFilter).join(" OR ");
  const { field, op, value } = tree;
  const valStr = Array.isArray(value) ? value.join(", ") : String(value ?? "");
  return `${field} ${op}${valStr ? " " + valStr : ""}`;
}

/* ---------------- Operator catalog (for UI) ---------------- */

export interface OperatorDef {
  op: FilterOp;
  label: string;
  /** How many values the operator takes. `0` = unary, `1` = scalar, `2` = [min,max]. */
  arity: 0 | 1 | 2;
  /** Kinds of fields this operator applies to. */
  supports: readonly (
    | "text"
    | "number"
    | "currency"
  ...
  /** Group it into in the UI (for organization). */
  group: "compare" | "text" | "range" | "date" | "set" | "null";
}
...
export function operatorsFor(kind: string): OperatorDef[] {
  return OPERATORS.filter((o) => (o.supports as readonly string[]).includes(kind));
}
