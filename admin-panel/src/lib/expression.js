...
const CACHE = new Map();
export function evalExpression(expr, record) {
    if (!expr)
    ...
    if (typeof ast === "string")
        return { value: undefined, error: ast };
    try {
...
export function evalExpressionNumeric(expr, record) {
...
const FUNCS = {
    ...
    round: (x, places) => Number(Math.round(toNum(x) * 10 ** toNum(places ?? 0)) / 10 ** toNum(places ?? 0)),
    floor: (x) => Math.floor(toNum(x)),
    ceil: (x) => Math.ceil(toNum(x)),
    sum: (...xs) => xs.reduce((a, b) => a + toNum(b), 0),
    avg: (...xs) => (xs.length ? xs.reduce((a, b) => a + toNum(b), 0) / xs.length : 0),
    len: (x) => {
        if (Array.isArray(x))
            return x.length;
        if (typeof x === "string")
...
function run(node, rec) {
    switch (node.t) {
        ...
        case "bin": {
            ...
            if (node.op === "&&")
                return toBool(a) ? run(node.b, rec) : a;
            if (node.op === "||")
                return toBool(a) ? a : run(node.b, rec);
